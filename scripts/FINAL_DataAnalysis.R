...
counts_validation_batchnorm <- counts_validation[row.names(counts_validation) %in% unbiased_genes,]

# Only count mRNA from exonic regions
counts_validation_batchnorm_mRNA <- filterCountsbyGeneType(counts_validation_batchnorm, annotation_path, "exon", c("mRNA"))

#Normalize the counts 
counts_validation_batchnorm_vst <- DESeq2::vst(as.matrix(counts_validation_batchnorm_mRNA))

#Add Kraken Data
metadata_validation_filtered <- dplyr::left_join(metadata_validation, kraken_data, by="Sample_ID")
...
prediction_guesses<-
prediction_guesses %>%
  mutate(guess_isCluster_1 = ifelse(Type != "DFU" & Bac_prcnt < 10, "1", "0"))

#Guess if C2

prediction_guesses %>%
  mutate(guess_isCluster_2 = ifelse(Type == "DFU" & Bac_prcnt < 10, "1", "0"))

#Guess if C3
prediction_guesses<-
  prediction_guesses %>% 
  mutate(guess_isCluster_3 = ifelse(Bac_prcnt > 10, "1", "0"))
...
# Add dummy for being in C3

#metadata_validation_filtered$is_C3 <-ifelse(metadata_validation_filtered$cluster_res_all == "3", "yes", "no")
...
write.csv(metadata_validation_filtered, "./data/Example_data/Validation_metadata.csv", row.names = F, quote = TRUE)
